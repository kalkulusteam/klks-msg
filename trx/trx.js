...
class BitJS {
    ...
    static hexToBytes(b) { for (var a = [], c = 0; c < b.length; c += 2)
        a.push(parseInt(b.substr(c, 2), 16)); return a; }
    static bytesToHex(b) { for (var a = [], c = 0; c < b.length; c++)
    ...
    static bytesToNum(bytes) {
        if (bytes.length == 0)
            ...
            return bytes[0] + 256 * this.bytesToNum(bytes.slice(1));
    }
    /* clone an object */
    static clone(obj) {
